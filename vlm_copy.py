...
class DecoderLanguageModel(nn.Module):
    def __init__(
        ...
        self.value_head = nn.Linear(n_embed, 1)
    ...
    def forward(
        ...
        values = self.value_head(x).squeeze(-1)  # shape: (B, T) 

        if targets is not None:
            if self.use_images and img_embeds is not None:
                batch_size = idx.size(0)
                targets = torch.cat(
                    [torch.full((batch_size, 1), -100, dtype=torch.long, device=idx.device), targets],
                    dim=1,
                )
        ...
        return logits, values  # Return hidden state (B, T, D)
...
class VisionLanguageModel(nn.Module):
    ...
    def forward(
        ...
        if targets is not None:
            ...
            logits, values, loss = self.decoder(idx, image_embeds, targets)
            return logits, values, loss
        else:
            # If targets are not provided, compute only the logits
            logits, values = self.decoder(idx, image_embeds)
